...
export const otpChallenges = sqliteTable(
	"otp_challenges",
	{
		...
		identifierType: text("identifier_type", { enum: ["email", "phone"] }).notNull(),
	...
	(table) => ({
		identifierIdx: index("otp_challenges_identifier_idx").on(table.identifierType, table.identifierValue),
	}),
);
