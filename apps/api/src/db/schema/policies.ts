...
export const policies = sqliteTable(
	"policies",
	{
		...
		locationEnabled: integer("location_enabled", { mode: "boolean" }).notNull().default(true),
		notificationsEnabled: integer("notifications_enabled", { mode: "boolean" })
			.notNull()
			.default(true),
		excludeFromPermissive: integer("exclude_from_permissive", { mode: "boolean" })
			.notNull()
			.default(false),
		updatedAt: integer("updated_at").notNull().default(sql`(unixepoch())`),
	},
	(table) => ({
		ownerScopeTargetUnique: uniqueIndex("policies_owner_scope_target_unique").on(
			table.ownerId,
			table.scope,
			table.targetId,
		),
